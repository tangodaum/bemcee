...
# sns.set_style("ticks")  # Options: whitegrid, ticks, talk, white, paper
...
def corner(xs, bins=20, range=None, weights=None, color="k",
           data_file=None, star=None, folder_fig=None,
           computer=None, sns_plot=False, lbd_range=None,
...
corner
    ...
    for i, x in enumerate(xs):
        ...
        if smooth1d is None:
            # if sns_plot is True:
            #     sns.kdeplot(x, ax=ax, shade=True,
            #                 color='blue', shade_lowest=True)

            # else:
            n, _, _ = ax.hist(x, bins=int(bins[i]), weights=weights,
                              range=np.sort(range[i]), **hist_kwargs)
            ax.get_yaxis().set_tick_params(direction='in', size=3)
            ax.get_xaxis().set_tick_params(direction='in', size=3)

        else:
            if gaussian_filter is None:
                raise ImportError("Please install scipy for smoothing")
            n, b = np.histogram(x, bins=int(bins[i]), weights=weights,
        ...
        if show_titles:
            ...
            best_values.append([q_m, q_50, q_p])  # Bruno *****
            os.chdir(folder_fig + star +  '_' + lbd_range + '/')

            file = open(data_file, "a")
            # file.write("-"*72)
            # file.write('\n%0.0f Result\n' % (i+1) )
            # file.write('True Equatorial Radius: %0.2f\n' % Req_true)
            # file.write('%s \t\t$%0.2f_{-%0.2f}^{+%0.2f}$\n' %
            #            (labels[i], q_50, q_m, q_p))
            file.write('\t\t$%0.2f_{-%0.2f}^{+%0.2f}$\n' %
                       (q_50, q_m, q_p))
            file.close()

            # Add in the column name if it's given.
            if labels is not None:
                # title = "{0} = {1}".format(labels[i], title)  # bruno
                title = "{0}".format(title)  # bruno
        ...
        ax.tick_params(labelsize=16)  # Bruno
        ax.get_yaxis().set_tick_params(direction='in', size=3)
        ax.get_xaxis().set_tick_params(direction='in', size=3)
        ...
        if i < K - 1:
            ...
            [l.set_rotation(45) for l in ax.get_xticklabels()]
            if labels is not None:
                ax.set_xlabel(labels[i], fontsize=fontsize, **label_kwargs)
                ax.xaxis.set_label_coords(0.5, -0.3)
                ax.tick_params(labelsize=14)  # Bruno
        ...
        for j, y in enumerate(xs):
            ...
            # if sns_plot is False:
            hist2d(y, x, ax=ax, range=[range[j], range[i]],
                   weights=weights, color=color, smooth=smooth,
                   bins=[bins[j], bins[i]], **hist2d_kwargs)
            # MUDEI DAQUI BRUNO ***
            # else:
            #     data = list(zip(y, x))
            #     data = np.array(data)
            #     sns.kdeplot(data, shade=True, n_levels=5, cmap="Blues",
            #                 shade_lowest=False, ax=ax)
            # MUDEI ATEH AQUI **** BRUNO
            ax.get_yaxis().set_tick_params(direction='in', size=3)
            ax.get_xaxis().set_tick_params(direction='in', size=3)
...
def hist2d(x, y, bins=20, range=None, weights=None, levels=None, smooth=None,
    ...
    # density_cmap = LinearSegmentedColormap.from_list(
    #     "inferno", [color, (1, 1, 1, 0)])
    # density_cmap = phc.gradColor(np.arange(len(file_name)), cmapn='inferno')
